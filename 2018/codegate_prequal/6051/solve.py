from ctypes import *

s = [
	
]
ans = [
	
]
rle_decoded = ""
libc = CDLL("libc.so.6")

idx = 0
while idx < len(ans):
	c = ans[idx]
	if c == 0x16:
		size = s.index(ans[idx + 1])
		val = ans[idx + 2] - (libc.rand() & 0xff)
		for i in range(size):
			rle_decoded += chr(0x30 + val)
		idx += 3
	else:
		rle_decoded += chr(c - (libc.rand() & 0xff) + 0x30)
		idx += 1

bin_len = len(rle_decoded)
string = ['' for i in range(bin_len)]
for i in range(bin_len):
	for j in range(bin_len):
		string[j] = rle_decoded[j] + string[j]
	string.sort()
for i in range(bin_len):
	try:
		string[i] = str(hex(int(string[i], 2)))[2:-1].decode('hex')
	except:	# for Odd-Length Exception
		pass

for i in string:
    if i.startswith("FLAG{"):
        print i

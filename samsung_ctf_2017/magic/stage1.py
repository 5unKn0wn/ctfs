...
s = Solver()
# v9 = []
v9 = [BitVec('v9_%d' % i, 64) for i in range(4)]
# v4 = 0
v4 = [BitVec('v4_%d' % i, 64) for i in range(4)]
v5 = 0
...
for i in range(4):
	s.add(v9[i] != 0)
	s.add(v9[i] == LShR(((table1[i] * serial) & 0xffffffffffffffffffffffffffffffff), 64))
...
for i in range(4):
	s.add(v4[i] != 0)
	s.add(v4[i] == (serial - v9[i]) & 0xffffffffffffffff)
